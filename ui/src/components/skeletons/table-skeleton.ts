export class TableSkeleton extends HTMLElement {
  #root: ShadowRoot;
  ...
  render() {
    this.#root.innerHTML = `
      <style>
        ...
        .row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; padding: 10px 0; }
      </style>
      <div class="card" style="padding: 12px;">
        ${Array.from({ length: 5 }).map(() => `<div class="row"><div class="skel"></div><div class="skel"></div><div class="skel"></div></div>`).join('')}
      </div>
    `;
  }
}
