...
export const api = {
  async listApplications(): Promise<Application[]> {
    const data = await request<unknown>('/api/v1/applications');
  ...
  getApplication(id: string): Promise<Application> {
    return request<Application>(`/api/v1/applications/${encodeURIComponent(id)}`);
  },
  createApplication(input: Pick<Application, 'id' | 'name' | 'comments'>): Promise<Application> {
    return request<Application>('/api/v1/applications', { method: 'POST', body: JSON.stringify(input) });
  },
  updateApplication(id: string, input: Partial<Pick<Application, 'name' | 'comments'>>): Promise<Application> {
    return request<Application>(`/api/v1/applications/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(input) });
  },
  getConfiguration(id: string): Promise<Configuration> {
    return request<Configuration>(`/api/v1/configurations/${encodeURIComponent(id)}`);
  },
  createConfiguration(input: { id: string; application_id: string; name: string; comments?: unknown; config: Record<string, unknown> }): Promise<Configuration> {
    return request<Configuration>('/api/v1/configurations', { method: 'POST', body: JSON.stringify(input) });
  },
  updateConfiguration(id: string, input: Partial<Pick<Configuration, 'name' | 'comments' | 'config'>>): Promise<Configuration> {
    return request<Configuration>(`/api/v1/configurations/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(input) });
  }
};
